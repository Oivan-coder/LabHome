...
// Безопасная функция создания элемента с HTML (только для контролируемых данных)
function createElementWithSafeHTML(tag, className, html) {
    const element = document.createElement(tag);
    if (className) element.className = className;
    // Используем только для статических данных, которые мы контролируем
...
function openSpecialistModal(specialistId) {
    ...
    specialist.contacts.forEach(contact => {
        ...
        link.href = createSafeUrl(contact.type, contact.value);
...
document.addEventListener('DOMContentLoaded', function() {
    debugLog('🚀 Инициализация Форума лабораторных инноваций');
...
function initSpeakersCarousel() {
    ...
    debugLog('🎯 Инициализация карусели спикеров...');
    ...
    speakers.forEach((speaker, index) => {
        ...
        photoDiv.style.backgroundImage = `url('${escapeHtml(speaker.photo)}')`;
